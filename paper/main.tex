...
\begin{enumerate}
    ...
    \item For initial sampling $D_0$ update the posterior distribution of the objective $f$
...
As the first step one defines the prior distribution for an initial input $D_0 = x_{1:n} = (x_1,..., x_n)$
...
Given the previous iteration step head $n$ points $\mathbf{x}_{1:n} = (x_1, ..., x_n)$
adding a new sampling point $\tilde x$ gives the conditional distribution of $f(x)$~\cite{rasmussen_gaussian_2006}:
